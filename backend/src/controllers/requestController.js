...
const acceptRequest = async (req, res = response) => {
  ...
  try {
    ...
    // 1. fetch and lock the data
    const requestCheck = await client.query(
      `SELECT r.*, i.donor_id, i.status AS item_status, i.title AS item_name,
              i.pickup_instructions, u_donor.email AS donor_email, u_requester.email AS requester_email 
       FROM requests r 
    ...
    if (requestCheck.rows.length === 0) {
      await client.query('ROLLBACK');
      return res.status(404).json({ ok: false, message: 'Request not found (TOT)' });
    }
    ...
    if (requestData.donor_id !== donor_id) {
      await client.query('ROLLBACK');
      return res.status(403).json({ ok: false, message: 'Not your item!' });
    }
    
    if (requestData.status !== 'pending') {
      await client.query('ROLLBACK');
      return res.status(400).json({ ok: false, message: 'Only pending requests can be accepted' });
    }

    if (requestData.item_status !== 'available') {
      await client.query('ROLLBACK');
      return res.status(409).json({ ok: false, message: 'Item is no longer available' });
    }
    
    // 3. generate 6 digit code
    const autoGeneratedCode = Math.floor(100000 + Math.random() * 900000).toString();
    
    // 4. update item to reserved
    await client.query(
      "UPDATE donation_items SET status = 'reserved', claim_code = $1, updated_at = NOW() WHERE id = $2",
      [autoGeneratedCode, requestData.item_id]
    );
    ...
    // 6. reject all other pending requests for this item
    await client.query(
      "UPDATE requests SET status = 'rejected', updated_at = NOW() WHERE item_id = $1 AND id != $2 AND status = 'pending'",
      [requestData.item_id, request_id]
    );
    ...
    return res.status(200).json({
      ok: true,
      message: 'Request accepted successfully! (^u^)',
    ...
    return res.status(500).json({ ok: false, message: 'Internal server error (-L-)' });
  } finally {
...
const claimItem = async (req, res=response) => {
  const { request_id } = req.params;
  ...
  try {
    ...
    //fetch the request and the item's secret code
    const requestQuery = await client.query(
      `SELECT r.id, r.item_id, r.status, i.claim_code as secret 
       FROM requests r 
       JOIN donation_items i ON r.item_id = i.id 
    ...
    if (request.status === 'completed') {
      return res.status(400).json({ ok: false, msg: 'This item has already been claimed!' });
    }

    // validation->does the typed code match the DB secret?
    if (request.secret !== claim_code) {
      return res.status(400).json({ ok: false, msg: 'Invalid claim code! Please ask the donor for the 6-digit PIN.' });
    }

    // start transaction to flip the statuses
    await client.query('BEGIN');

    // mark the item as CLAIMED
    await client.query(
      "UPDATE donation_items SET status = 'claimed', updated_at = NOW() WHERE id = $1",
      [request.item_id]
    );
    ...
    res.json({ 
      ok: true, 
      msg: 'Success! You have officially claimed the item. (^_^)' 
    });
